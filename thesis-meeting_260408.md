...
- **변수 단위 선택의 명확화:** 기존 prototype의 element-wise $\ell_1$ penalty는 하나의 변수 안에서도 일부 군집 효과만 0이 되는 파편화를 유발할 수 있었다. 현재는 변수 $k$ 에 대응하는 군집 편차 벡터 전체 $\delta_{\cdot k}$ 에 Adaptive Group Lasso penalty를 부여하여 변수 단위의 선택을 유도한다.
    
- **식별성 제약의 안정화:** 혼합비율에 의존하는 $\sum_{j=1}^K \pi_j \delta_j = 0$ 대신
    
    $$\sum_{j=1}^K \delta_{jk} = 0,\qquad k=1,\dots,p$$
    
    제약을 사용하여 sum-to-zero coding에 의한 effects parameterization을 채택하였다. 단, 이 제약은 $\pi_j$ 가 균등할 때에 한해 ANOVA형 grand mean과 일치하며, 일반적인 경우 $\mu_0$ 는 marginal mean과 다른 기준점 역할을 한다.
    
- **제약을 보존하는 재파라미터화:** 직교여공간 basis $Q$ 를 이용해
    
    $$\delta_{\cdot k}=Q\alpha_k$$
    
    로 재파라미터화하면, 제약을 자동으로 만족시키면서 희소성을 안정적으로 유지할 수 있다.
    
- **adaptive penalty의 반영:** 참고 논문은 non-adaptive보다 adaptive regularization이 변수선택 성능에서 더 안정적임을 보인다. 현재 제안 모형은 이 점을 반영하여 adaptive group penalty를 중심으로 정리한다.
...
- **분해의 유일성:** sum-to-zero 제약 $\sum_{j=1}^K \delta_{jk} = 0$ 하에서 $\mu_0$ 와 $\delta_j$ 의 분해는 유일하다. 이는 대수적으로 직접 확인된다.
    
- **혼합모형의 식별성:** label switching을 제외하고 서로 다른 파라미터 $(\pi, \mu_0, \Delta, \Sigma)$ 가 동일한 mixture distribution을 생성하지 않음을 별도로 보여야 한다. 이를 위해 Gaussian mixture의 일반적 식별성 조건(예: 성분 수 $K$ 가 주어진 경우의 generic identifiability)을 인용하거나, 본 모형의 파라미터화 하에서 직접 검증한다.
    

**둘째, 추정오차 경계.** 희소도 $s=|S_H|$ 에 대해 다음과 같은 형태의 오차 경계를 목표로 한다.

$$\|\hat{\Delta}-\Delta^*\|_F = O_p\left(\sqrt{\frac{s(K-1)\log p}{n}}\right)$$
...
**넷째, clustering performance.** 두 목표는 서로 다른 가정 체계를 요구하므로, 다음과 같이 구분하여 설정한다.

- **Regime 1 (fixed separation):** component overlap이 존재하는 일반적인 상황에서는 Bayes classifier 자체도 양의 오분류율을 가진다. 따라서 이 경우의 목표는 $R(\hat{g}) - R(g^*) \to 0$ 과 같은 excess risk consistency로 설정한다. 이는 추정된 군집 규칙이 최적 Bayes 규칙에 수렴함을 의미한다.
    
- **Regime 2 (growing separation):**
    
    $$\Delta_{\min,n}^2 = \min_{j \neq \ell} \sum_{k \in S_H} (\delta_{jk}^* - \delta_{\ell k}^*)^2 / \sigma_k^2 \to \infty$$
    
    를 추가로 가정하는 stronger regime에서는
...
본 발표의 표는 $R=10$ 회의 pilot Monte Carlo 평균을 기반으로 한다. 아래 표들은 동일 알고리즘을 저차원, 고차원, 초고차원 환경으로 확장한 반복 실험 결과를 정리한 것이다.
...
앞의 $q$ 개 변수만 signal variables이며, 나머지는 noise variables이다. 군집별 편차 벡터는 대칭 구조 $(a,0,-a)$ 를 따르도록 설계한다.

**3) 공분산 구조**

$$X_i \mid Z_i=j \sim N_p(\mu_j,\Sigma)$$

이며, 본문 시뮬레이션에서는 signal variables의 분산을 $1$, noise variables의 분산을 $2$ 로 설정한다. 생성된 최종 데이터는 변수별 중심화만 수행한다.

### 3. 평가 지표

선택 성능은

$$\mathrm{TPR} = \frac{|S_H\cap \hat{S}_H|}{|S_H|} ,\qquad \mathrm{FPR} = \frac{|\hat{S}_H\setminus S_H|}{p-|S_H|}$$

로 정의한다. 여기서 $\hat{S}_H$ 는 선택된 mean-heterogeneity 변수 집합이다. 표에서는 $\hat{S}$ 로 표기한다.
...
- **낮은 FPR과 높은 TPR 유지:** HP는 저차원·고차원·초고차원 환경 모두에서 mean-heterogeneity variables를 비교적 안정적으로 복원하는 경향을 보였다.
...
- **pilot 결과라는 점을 명시할 필요:** 현재는 $R=10$ 반복의 pilot Monte Carlo 결과이므로, 더 큰 반복 수와 보조 시나리오에서 재검증이 필요하다.
...
- **반복 수 확대:** 현재 $R=10$ pilot Monte Carlo를 $R=200$ 또는 $R=500$ 수준으로 확대하고, 평균과 표준오차를 함께 보고한다.
    
- **correlated predictors 실험:**
...
- **all-heterogeneous setting:** 모든 informative variables가 heterogeneous한 경우에는 heterogeneity pursuit의 이점이 약해질 수 있으므로, 이 구간도 점검한다.
    
- **adaptive vs non-adaptive 비교:** 참고 논문과 동일한 메시지, 즉 adaptive penalty가 non-adaptive보다 더 안정적이라는 점을 mean-mixture setting에서도 검토한다.
    
- **군집 수 선택:** 현재는 $K$ 를 고정했지만, 최종 논문에서는 $K$ 와 penalty parameter를 함께 선택하는 방향으로 확장한다.
