...
| Diagnostic | 주요 설계 | 주요 결과 | 해석 |
|:---|:---|:---|:---|
| Direction-sparse metric diagnostic (old A smoke) | $K=4$, $d=100$, true union $q=20$, $\kappa=(60,60,60,60)$ | Eta + refit ARI=0.976, selected q=28.8, F1=0.676; Rossi/Separate ARI=0.975, selected q=100, F1=0.333 | Rossi/Separate가 뚜렷하게 유리한 setting은 아니었다. Eta가 support recovery는 더 좋았지만, Eta refit의 MSE_kappa가 컸다. |
| Dense-support negative control (old B smoke) | $K=4$, $d=100$, true union $q=80$, $\kappa=(30,45,65,90)$ | Eta + refit ARI=0.377, selected q=47.4, F1=0.702; Separate + refit ARI=0.396, selected q=100, F1=0.889 | 좋은 negative-control 후보. True support가 dense하면 Eta-group이 과도하게 shrink하여 ARI/F1 손실이 생긴다. |
| Weak-signal stress smoke (old C smoke) | $K=4$, $d=100$, $w=0.20$, $\kappa=(25,30,35,40)$ | Eta BIC selected q=0, refit invalid; Rossi/Separate ARI near 0 | 너무 어려운 low-signal stress setting이다. 거의 모든 방법이 실패한다. |

## 3. Dense-support negative control (old Setting B, rep50)
...
- Dense-support negative control은 현재까지 가장 명확한 Eta-group failure mode다.
...
## 4. Weak-signal tuning diagnostic (old C2, rep50)
...
- Weak-signal tuning diagnostic에서도 weak-signal 구조는 여전히 어렵다. Rossi/Separate는 ARI가 약 0.13-0.14 수준이고 support는 거의 full support로 선택된다.
...
- 따라서 Weak-signal tuning diagnostic은 Rossi/Separate가 깨끗하게 유리한 setting이라기보다, Eta BIC의 zero-support tuning failure와 weak-signal clustering difficulty를 보여주는 diagnostic으로 보는 것이 맞다.

## 5. Support-target diagnostics

이 진단들은 Rossi/Separate와 Eta-group이 같은 support target을 목표로 하는지 확인하기 위한 것이다. Coordinate union support만 보면 Eta-group이 유리해 보일 수 있고, prototype entry support를 보면 Rossi/Separate의 장점이 일부 드러날 수 있다.

### 5.1 Direction-sparse metric diagnostic (old A2)
...
- Direction-sparse metric diagnostic도 현재 coordinate union support 기준에서는 Rossi/Separate가 명확히 유리한 결과를 만들지 못했다.
...
- 따라서 이 결과는 방법의 명확한 실패라기보다 metric mismatch를 보여준다. Rossi-style direction sparsity를 공정하게 평가하려면 prototype/entry-level support recovery와 posterior decision support를 분리해야 한다.

### 5.2 Entry-sparse / union-dense diagnostic (old A4)

Entry-sparse / union-dense diagnostic은 기존 generator를 수정하지 않고 만들 수 있는 가장 직접적인 redesign이다. 공통 좌표를 제거하고, 각 component가 서로 겹치지 않는 20개 좌표만 사용하도록 했다.
...
- Entry-sparse / union-dense diagnostic에서도 ARI는 모든 방법에서 거의 동일하게 높다.
...
- 따라서 이 결과는 Rossi/Separate가 전체적으로 우월한 setting이라기보다, posterior decision support와 prototype entry support를 분리해서 보고해야 한다는 점을 더 분명하게 보여준다.
...
### 5.3 Fragmented-support diagnostic

Rossi/Separate가 유리할 수 있는 setting으로, 공통 support를 0으로 두고 component-specific support만 사용하는 fragmented-support 구조를 현재 generator 안에서 확인했다. 완전한 binary block-diagonal generator는 아니지만, R core simulation code를 수정하지 않고 만들 수 있는 가장 가까운 diagnostic이다.

#### 5.3.1 저차원 fragmented-support smoke
...
#### 5.3.2 고차원 fragmented-support smoke
...
d=800 fragmented-support smoke도 common q=0, specific q=20 per component로 시도했다. Replicate 계산은 진행됐지만 summary binding 단계에서 column mismatch error가 발생했다. 이번 diagnostic에서는 R core algorithm을 수정하지 않기로 했으므로 이 결과는 해석에 사용하지 않는다. 대신 very high-dimensional diagnostic에서 script robustness issue가 있음을 기록한다.

## 6. Support metric 정리

Support-target diagnostics의 핵심은 세 가지 support target을 분리해서 해석해야 한다는 점이다.
...
여기서 $c_{kj}=\eta_{kj}-K^{-1}\sum_\ell \eta_{\ell j}$이다. Entry-sparse / union-dense diagnostic은 이 세 지표가 같은 결론을 주지 않을 수 있음을 보여준다.

## 7. 현재 결론

현재까지 가장 유용한 negative-control 결과는 Dense-support negative control이다.

Dense-support negative control은 Eta-group이 보편적으로 더 좋은 방법이 아님을 보여준다. True separation이 dense하고 많은 weak coordinate가 함께 작동하는 경우 Eta-group은 support를 과도하게 shrink하여 Rossi 또는 Separate보다 ARI/F1이 낮아질 수 있다. 이 결과는 논문에서 limitation 또는 negative-control diagnostic으로 포함할 가치가 있다.

Weak-signal tuning diagnostic은 weak signal에서 Eta BIC가 zero support를 자주 선택할 수 있음을 보여준다. 50회 중 43회에서 selected q=0이었고, BIC-selected refit valid replicate는 7회뿐이었다. 따라서 positive-support 또는 alternative tuning은 diagnostic으로는 유용하지만 official tuning으로 제시하면 안 된다.

Direction-sparse metric diagnostic은 equal-concentration direction-sparse design에서도 coordinate union support 기준으로는 Eta-group이 여전히 유리하게 보인다는 점을 보여준다. 이는 Rossi/Separate가 더 나을 수 없다는 뜻이 아니라, prototype-level target과 decision-level target을 분리해야 한다는 뜻이다.

Entry-sparse / union-dense diagnostic도 같은 방향이다. Union-dense / entry-sparse 구조를 만들었지만 coordinate union support 기준에서는 Eta-group이 여전히 더 좋고, prototype entry support에서는 Separate가 더 나아 보인다. 따라서 이 setting은 clean failure라기보다 metric separation diagnostic으로 보는 것이 맞다.

Fragmented-support diagnostic도 현재 generator에서는 clean Rossi/Separate-dominant result를 만들지 못했다. 모든 방법이 clustering을 잘하거나 dense support로 가며, prototype-sparsity advantage가 분리되지 않는다. Rossi/Separate가 구조적으로 유리한 setting을 보이려면 dedicated block-diagonal generator가 필요하다.

## 8. Full simulation 권장 사항

| 후보 | 권장 여부 | 이유 |
|:---|:---|:---|
| Dense-support negative control rep100 | 선택 사항, 급하지 않음 | rep50만으로도 안정적인 negative-control evidence가 있음 |
| Weak-signal tuning diagnostic rep50 | 완료 | Eta BIC zero-support tuning failure를 확인했지만, Rossi/Separate도 dense support이므로 cleaner weak-signal negative-control은 별도 설계가 필요함 |
| Entry-sparse / union-dense diagnostic rep50 | 조건부 보류 | entry-level에서는 Separate가 유리해 보이나 union support에서는 Eta-group이 여전히 유리함. 먼저 prototype-support metric 정의가 필요함 |
