...
H_K
=
I_K-\frac{1}{K}\mathbf1_K\mathbf1_K^\top
$$
...
$$
\log
\frac{P(Z=k\mid x)}{P(Z=\ell\mid x)}
=
\log
\frac{\alpha_kC_d(\kappa_k)}
...
=
(
\lVert c_{\cdot j}^{\mathrm{init}}\rVert_2+\epsilon
)^{-\gamma}
$$
...
\mu_k
=
\frac{r_k}{\lVert r_k\rVert_2}
$$

$$
\rho_k
=
\frac{\lVert r_k\rVert_2}{N_k}
$$

$$
\kappa_k
\approx
\widehat\kappa_B(\rho_k)
$$
...
\min_E
[
-\sum_{k=1}^K r_k^\top\eta_k
-\sum_{k=1}^K N_k\log C_d(\lVert\eta_k\rVert_2)
+
\lambda_\eta
\sum_{j=1}^d
w_j
...
=
-\sum_k r_k^\top\eta_k
-\sum_kN_k\log C_d(\lVert\eta_k\rVert_2)
$$
...
$$
\widehat S_\eta(\lambda)
=
\{
j:
\lVert H_K\widehat E_{\cdot j}\rVert_2
>
\mathrm{tol}
\}
$$
...
$$
\max_{\lVert\mu_k\rVert_2=1}
[
\sum_k\kappa_kr_k^\top\mu_k
-
\lambda_\mu
\sum_jw_j
\lVert H_KM_{\cdot j}\rVert_2
