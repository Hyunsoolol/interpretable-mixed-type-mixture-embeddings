...
단위구면 제약과 비매끄러운 group penalty를 분리한다.
...
ADMM 내부에서 $\mu$는 product of spheres 위에서 갱신하고, $Z$에는
coordinate-wise group soft-thresholding을 적용한 뒤 두 변수의 일치 오차를
dual update로 줄인다.
...
를 사용한다. Banerjee et al. (2005)의 근사값
...
은 수치적 근 탐색구간의 초기화에만 사용하며, M-CGL과 M-ACGL의 최종
$\kappa_k$는 $A_d(\kappa_k)=\rho_k$의 수치해이다.
...
| 10 | $\kappa$-update: Banerjee 근사값으로 탐색구간을 초기화하고 $A_d(\kappa_k)=r_k^{\mathsf T}\mu_k/N_k$의 수치적 근 계산 |
