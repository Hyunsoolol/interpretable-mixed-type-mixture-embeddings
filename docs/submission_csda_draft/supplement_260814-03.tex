...
not updated along the path. This construction applies the adaptive
regularization principle of \citet{zou2006adaptive} to centered coordinate
groups. No oracle-property claim is made.
...
diagnostic, four replicate-level dense free-concentration pilots reached their
iteration limits. All selected E-series fits
...
For target $e_B$, scenario index $h\in\{1,2\}$ (equal, heterogeneous), sample
size $n$, and repetition $b$, the prespecified grid uses
\[
\begin{aligned}
s_{\mathrm{cal}}&=271828182+100\,\operatorname{round}(1000e_B)+h,\\
s_{\mathrm{cell}}&=314159265+100000\,\operatorname{round}(1000e_B)+10000h+n,\\
s_{\mathrm{data},b}&=s_{\mathrm{cell}}+10000+b,\\
s_{\mathrm{init},b}&=s_{\mathrm{cell}}+20000+b.
\end{aligned}
\]
The same calibrated data-generating process (DGP) is therefore used at $n=300$
and $n=1000$ for each
target-error and concentration combination. Within a repetition, all methods
receive the same sample and the same 10-start dense initialization.
...
The Rossi-style entrywise mean-direction lasso (M-L) uses its event-driven
path with at most 240 updates in the
primary simulations. E-CGL and E-ACGL use the zero-augmented 240-point
...
The finite vMF mixture likelihood can diverge under concentration collapse.
...
fit is finite and nonfailed but unconverged, allows 440 additional iterations
in the recorded simulation and real-data protocols. Each conditional refit M-step uses the
...
The recorded Classic3 full-data paths and all ten split-specific E-series paths
each contain all 240 configured points and end with support size 2--4, so the
support-size early stop does not truncate them. Across their 2,880 path points,
66 reached the original 100-iteration limit; none generated the source support
of a selected refit. Removing those unconverged points therefore leaves each
selected Classic3 support unchanged. Every selected Classic3 refit is eligible
and converged, and every path point satisfies the recorded majorization check.
...
(4,160,36),
\]
respectively. Both have achieved oracle error approximately 0.10. The
...
weighting provided an additional improvement. E-CGL remains the primary
formulation because its selection unit is the cross-component coordinate
contrast $c_{\cdot j}$; E-ACGL remains an adaptive extension, and the ablation
does not imply uniform superiority over all regimes.

\section{Selector sensitivity}
\label{sec:s-selector-sensitivity}

A proximal-path sensitivity diagnostic compared BIC before and after refit,
the main nominal dimension
$\operatorname{df}_A(S):=\operatorname{df}_E(S)$ in
\eqref{eq:supp-df-e},

the active-only sensitivity dimension
...
and EBIC with
$\gamma_{\mathrm{EBIC}}\in\{0.25,0.5,1\}$. These dimensions are practical
penalty slopes rather than exact effective degrees of freedom.
Table~\ref{tab:s-selector-sensitivity} shows the most difficult
$n=300$ cell. This diagnostic uses five repetitions and is interpreted as a
selector audit rather than a performance experiment.
...
Table~\ref{tab:s-path-coverage} separates two events that a single selected-
support rate conflates. Path coverage asks whether the fitted path ever
...
down to $10^{-6}$ showed that the dense E-ACGL selection was a candidate-
coverage issue rather than evidence that adaptive weighting intrinsically
requires a dense support.

To retain one path rather than a dataset-specific union, the final analysis
uses a single $L=240$ path from $10^{-4}\widetilde\lambda_{\max}$ to
$\widetilde\lambda_{\max}$ for both E-series methods. The data, dense starts,
...
descriptive run records and are not compared with the end-to-end matched-
runtime panel.
...
fits were finite and converged. The numerical tables in this Supplementary Material use
these final single-path results; the preceding standard and two-segment
results are retained only in the path-audit tables.
...
criterion (AIC) and independent test NLL selected $K=4$ in every repetition.
The Bayesian information criterion (BIC) selected $K=4$ in 39/50 equal-
concentration repetitions and 0/50 heterogeneous repetitions. Extended BIC
(EBIC) with index 0.5, the risk inflation criterion (RIC), and its corrected
form (RICc) selected $K=2$ in all repetitions. A 10-repetition five-split
...
\operatorname{EBIC}_{0.5}(K)&=-2\ell_K+\{\log(n)+\log(d)\}\operatorname{df}_K.
\end{aligned}
\]

The RIC and RICc labels refer to these recorded high-dimensional penalty
forms; they are used only as component-number sensitivity diagnostics.
...
of multipliers (ADMM): it introduces an auxiliary matrix
...
therefore cannot prevent concentration collapse by itself. The directional
...
\input{../simulations/tables/csda_supp_realdata_five_split_results_floor1e4_260812.tex}
...
\input{../simulations/tables/csda_supp_realdata_paired_floor1e4_260812.tex}
...
training vocabularies was 0.933 under the final single path. Coordinates absent
...
The primary M-L path allowed 600 updates and imposed a minimum relative
penalty increment of 0.02. A sensitivity run allowed 1,200 updates and reduced
the minimum increment to 0.005. Results are summarized in
...
Bessel-ratio calculation was compared with the high-dimensional reference.
