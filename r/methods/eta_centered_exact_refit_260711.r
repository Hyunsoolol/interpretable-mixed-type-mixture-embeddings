# Reusable helpers for exact fixed-support centered-Eta refitting.
# Inactive coordinates retain an estimated common natural-parameter baseline:
# c[, j] = 0, equivalently eta[, j] = mean(eta[, j]).
...
eta_centered_support_df <- function(active, K, d = length(active)) {
  m <- sum(active)
  d + (K - 1L) * m + (K - 1L) * as.integer(m > 0L)
}

eta_centered_support_ic <- function(loglik, active, n, K,
                                    d = length(active), gamma = 0.5) {
  m <- sum(active)
  df <- eta_centered_support_df(active, K, d)
  bic <- -2 * loglik + log(n) * df
  ebic <- bic + if (m > 0L && m < d) 2 * gamma * lchoose(d, m) else 0
  data.frame(df = df, BIC = bic, EBIC = ebic)
}

fit_eta_centered_support_refit_exact <- function(
    X, active, init, e_step_fn = e_step_vmf, max_iter = 100L,
    optim_maxit = 100L, optim_factr = 1e7, optim_pgtol = 1e-6,
    abs_tol = 1e-7, rel_tol = 1e-8, min_step = 2^-10,
    loglik_tol = 1e-8) {
  X <- as.matrix(X)
  ...
  d <- ncol(X)
  n <- nrow(X)
  if (length(active) != d) stop("active must have length ncol(X).")

  eta0 <- sweep(init$mu, 1L, init$kappa, "*")
  eta0 <- exact_refit_project(eta0, active)
  theta <- exact_refit_eta_to_theta(init$alpha, eta0, init$mu)
  current <- e_step_fn(X, theta)
  ...
  for (iter in seq_len(max_iter)) {
    ...
    old_q <- exact_refit_q(eta_old, r, Nk)
    inner <- exact_refit_mstep(
      r, Nk, active, eta_old, maxit = optim_maxit,
      factr = optim_factr, pgtol = optim_pgtol
    )
    ...
    repeat {
      ...
      alpha_try <- pmax(alpha_try, 1e-12)
      alpha_try <- alpha_try / sum(alpha_try)
      candidate <- exact_refit_eta_to_theta(alpha_try, eta_try, theta$mu)
      next_e <- e_step_fn(X, candidate)
      if (is.finite(next_e$loglik) &&
          next_e$loglik >= old_loglik - loglik_tol) {
        accepted <- TRUE
        break
      }
    ...
    q_diff <- exact_refit_q(eta_try, r, Nk) - old_q
    min_loglik_diff <- min(min_loglik_diff, diff)
    min_q_diff <- min(min_q_diff, q_diff)
    theta <- candidate
    current <- next_e
    completed_iter <- iter
    if (abs(diff) <= abs_tol + rel_tol * abs(old_loglik)) {
      converged <- TRUE
  ...
  c(theta, list(
    ...
    constraint_error = exact_refit_constraint_error(
      sweep(theta$mu, 1L, theta$kappa, "*"), active
    ),
    refit_type = "exact_centered_support"
  ))
}
