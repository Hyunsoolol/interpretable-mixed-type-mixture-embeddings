...
# (기존 코드 틀 유지 + Element-wise L1 penalty 방법론 추가)
# ------------------------------------------------------------------------------
# 추가 내용
# - Element-wise L1 (mu0 + delta 구조, sum-to-zero 제약 없음):
#   각 delta_{jk}에 독립적으로 soft-thresholding 적용.
#   변수 선택은 delta_norms (group norm) 기준으로 후처리.
#   기존 Naive Lasso와의 차이: Naive Lasso는 sum-to-zero 제약을 inner loop에서
#   강제하지만, Element-wise L1은 제약 없이 각 delta_{jk}를 독립적으로 shrink.
# ==============================================================================
...
for (rep_idx in 1:n_rep) {
  ...
  for (scenario_idx in seq_along(signal_grid)) {
    ...
    # ==============================================================================
    cat("[3] Naive Lasso (sum-to-zero 제약 포함) 적합 중...\n")
      ...
      for (iter in 1:60) {
        ...
        for (j in 1:p) {
          ...
          for (inner in 1:10) {
            ...
            temp_delta <- temp_delta - mean(temp_delta)   # sum-to-zero 보정
    ...
    # ==============================================================================
    # Step 4-1. [NEW] Element-wise L1 (sum-to-zero 제약 없음) & Refit
    # ------------------------------------------------------------------------------
    # Naive Lasso와의 차이점:
    #   - inner loop에서 sum-to-zero 보정(temp_delta - mean(temp_delta))을 하지 않음
    #   - 각 delta_{jk}에 동일한 threshold를 독립적으로 적용
    #   - 변수 선택은 group norm ||delta_{.k}||_2 > threshold 기준으로 후처리
    #   - 이로 인해 동일 변수의 일부 군집 효과만 0이 되는 파편화(fragmentation)가
    #     발생할 수 있으며, 이것이 Group Lasso 계열과의 핵심 구조적 차이임
    # ==============================================================================
    ...
    for (lambda_ew in lambda_ew_grid) {
      ...
      for (iter in 1:60) {
        ...
        for (j in 1:p) {
          ...
          for (inner in 1:10) {
            mu0_j      <- sum(n_k_safe * (x_bar_j - delta_j)) / sum(n_k_safe)
            # [핵심 차이] sum-to-zero 보정 없이 각 delta_{jk}에 독립적으로 soft-thresholding
            temp_delta <- sign(x_bar_j - mu0_j) * pmax(0, abs(x_bar_j - mu0_j) - thresh_j)
            if (max(abs(temp_delta - delta_j)) < 1e-6) { delta_j <- temp_delta; break }
            delta_j <- temp_delta
          }
      ...
      # BIC degrees of freedom:
      # element-wise shrinkage이므로 nonzero delta_{jk} 개수를 직접 셈
    ...
    # 변수 선택 후처리: group norm 기준 (파편화 여부 확인 목적)
    ...
    method_names <- c(
      ...
      "Naive Lasso (L1 + sum-to-zero)",
      "Naive Lasso (L1 + sum-to-zero) + Refit",
      "Element-wise L1 (no constraint)",          # [NEW]
      "Element-wise L1 (no constraint) + Refit",  # [NEW]
      "Proposed: HP (Adaptive Group L2)",
      "Proposed: HP (Adaptive Group L2) + Refit",
      "Oracle-feature baseline (True Vars)",
      "True-parameter oracle"
    )
    
    used_dims_vec <- c(p, n_comp, p,
    ...
    lambda_num_vec <- c(NA, NA, NA,
                        NA, NA,
                        best_lambda_lasso, NA,
                        best_lambda_ew, NA,       # [NEW]
                        best_lambda_hp, NA,
                        NA, NA)
    
    ari_vec <- c(ari_km, ari_pca, ari_gmm,
                 ari_sparse, ari_sparse_refit,
                 ari_lasso,  ari_lasso_refit,
                 ari_ew,     ari_ew_refit,         # [NEW]
                 ari_hp,     ari_hp_refit,
    ...
    results_display <- data.frame(
      ...
      Penalty  = c("No", "No", "No",
                   "L1 Weights", "No",
                   "Element L1 + SumZero", "No",
                   "Element L1 (free)",    "No",  # [NEW]
                   "Adaptive Group L2",    "No",
                   "No", "No"),
...
for (i in 1:nrow(summary_keys)) {
  ...
  safe_se   <- function(x) { n <- sum(!is.na(x)); if (n > 1) sd(x, na.rm = TRUE)/sqrt(n) else NA_real_ }
...
combined_results_numeric$Method <- factor(
  combined_results_numeric$Method,
